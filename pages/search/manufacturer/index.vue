...
export default {
  ...
  async asyncData(context) {
    ...
    let countries = null
    await context.$axios
      .get(`/api/product/search/manufacturer?value=${context.query.value}`)
      .then((response) => {
  ...
  methods: {
    changePage(newPage) {
      ...
      this.$axios
        .get(
          `/api/product/search/manufacturer?value=${
  ...
  head() {
    return {
      title:
        '«СЕНАТОР» - поиск по производителю премиум алкоголя и подарков в Рязани',
      meta: [
        {
          hid: 'description',
          name: 'description',
