...
export default {
  ...
  async asyncData(context) {
    ...
    let countries = null
    await context.$axios
      .get(`/api/product/search/sugar?value=${context.query.value}`)
      .then((response) => {
  ...
  methods: {
    changePage(newPage) {
      ...
      this.$axios
        .get(
          `/api/product/search/sugar?value=${
  ...
  head() {
    return {
      title: '«СЕНАТОР» - поиск по количеству сахара премиум алкоголя в Рязани',
      meta: [
        {
          hid: 'description',
          name: 'description',
