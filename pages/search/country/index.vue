...
export default {
  ...
  async asyncData(context) {
    ...
    let countries = null
    await context.$axios
      .get(`/api/product/search/country?value=${context.query.value}`)
      .then((response) => {
  ...
  methods: {
    changePage(newPage) {
      ...
      this.$axios
        .get(
          `/api/product/search/country?value=${
  ...
  head() {
    return {
      title:
        '«СЕНАТОР» - поиск премиум алкоголя и подарков по стране производства в Рязани',
      meta: [
        {
          hid: 'description',
          name: 'description',
