...
export default {
  ...
  async asyncData(context) {
    let products = null
    await context.$axios
      .get(`/api/product/search/region?value=${context.query.value}`)
      .then((response) => {
  ...
  methods: {
    changePage(newPage) {
      ...
      this.$axios
        .get(
          `/api/product/search/region?value=${
  ...
  head() {
    return {
      title:
        '«СЕНАТОР» - поиск премиум алкоголя и подарков по объему бутылки в Рязани',
      meta: [
        {
          hid: 'description',
          name: 'description',
