...
export default {
  ...
  async asyncData(context) {
    let products = null
    await context.$axios
      .get(`/api/product/search/category?name=chocolate`)
      .then((response) => {
  ...
  methods: {
    changePage(newPage) {
      ...
      this.$axios
        .get(`/api/product/search/category?name=chocolate&page=${newPage - 1}`)
        .then((response) => {
  ...
  head() {
    return {
      title: '«СЕНАТОР» - купить элитный шоколад в бутике senator-wine.ru',
      meta: [
        {
          hid: 'description',
          name: 'description',
          content:
            'Элитный шоколад в бутике «СЕНАТОР»! Полный ассортимент, подробное описание, цены и скидки ☎ +7(4912)-28-19-37.'
        }
      ]
    }
  }
